...
- **No hardcoded values** — `Colors.xxx` / `fontSize:` / ハードコード文字列がないか。UI 表示文字列は `AppStrings` に集約（`'v$version'` のような補間文字列も `AppStrings.versionLabel(version)` のような static メソッドとして定義する）
