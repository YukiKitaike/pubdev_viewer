...
CLAUDE.md の Critical Rules と `/flutter-tips` スキル (Tip 1-14) に基づいて、変更されたコードをレビューする。
...
ルールの定義本体は CLAUDE.md の Critical Rules（詳細規約は `.claude/rules/` と各スキル）。
本エージェントの検出観点のみを以下に持つ:
...
- **No hardcoded values** — `Colors.xxx` / `fontSize:` / ハードコード文字列・補間文字列がないか（規約詳細は `/pubdev-ui` の「やってはいけないこと」）
- **No relative imports** — `lib/` 内で `import '../` パターンがないか
- **WHY comments only** — WHAT コメントが追加されていないか

## 2. Dart 言語スタイルチェック（flutter-tips Tip 1-6）
...
## 3. コード品質チェック（flutter-tips Tip 7-14）
