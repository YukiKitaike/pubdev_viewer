...
class _PackageListTileState extends State<PackageListTile> {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      // スクリーンリーダーでは「名前・バージョン・説明」を 1 ノードに統合して
      // 読み上げる。内部の個別テキスト・装飾は ExcludeSemantics で除外する。
      child: Semantics(
        button: true,
        container: true,
        label:
            '${widget.package.name}, '
            'バージョン ${widget.package.latest.version}。'
            '${widget.package.latest.pubspec.description}',
        hint: AppStrings.packageListItemHint,
        child: AnimatedScale(
          ...
          child: DecoratedBox(
            ...
            child: Material(
              ...
              child: InkWell(
                ...
                // 視覚表現用テキスト・装飾アイコンは親 Semantics.label に集約するため除外。
